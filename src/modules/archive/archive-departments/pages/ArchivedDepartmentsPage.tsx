...
import { useArchivedDepartments } from '@/hooks/useArchive';
import useDebounce from '@/hooks/useDebounce';
import type { ArchivedDepartment } from '@/types/archive.types';

import { useArchiveDepartmentSearchParams } from '../hooks/useSearch';

interface DepartmentTableData extends ArchivedDepartment {
  key: string;
}

export default function ArchivedDepartmentsPage() {
  const { searchData, setSearchData, resetSearchData } =
    useArchiveDepartmentSearchParams();
  const [searchName, setSearchName] = useState(searchData.search || '');
  const [first, setFirst] = useState(false);

  const debouncedName = useDebounce(searchName, 500);

  const { data: departmentsData, isLoading } = useArchivedDepartments({
    ...searchData,
  });
  ...
  const handleTableChange = (
    ...
    sorter:
      | SorterResult<DepartmentTableData>
      | SorterResult<DepartmentTableData>[],
  ) => {
    const updates: {
      page?: number;
