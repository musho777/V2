...
import { useArchivedBranches } from '@/hooks/useArchive';
import useDebounce from '@/hooks/useDebounce';
import type { ArchivedBranch } from '@/types/archive.types';

import { useArchiveBranchSearchParams } from '../hooks/useSearch';

interface BranchTableData extends ArchivedBranch {
  key: string;
}

export default function ArchivedBranchesPage() {
  const { searchData, setSearchData, resetSearchData } =
    useArchiveBranchSearchParams();
  const [searchName, setSearchName] = useState(searchData.search || '');
  const [first, setFirst] = useState(false);

  const debouncedName = useDebounce(searchName, 500);

  const { data: branchesData, isLoading } = useArchivedBranches({
    ...searchData,
  });
  ...
  const handleTableChange = (
    ...
    sorter: SorterResult<BranchTableData> | SorterResult<BranchTableData>[],
  ) => {
    const updates: {
      page?: number;
      size?: number;
